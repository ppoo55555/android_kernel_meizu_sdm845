		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		regmap_read(cs35l41->regmap, CS35L41_IRQ1_RAW_STATUS3, &val);
		if (val & CS35L41_PLL_CLK_EN_MASK)
...
static int cs35l41_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (cs35l41->pdata.dsp1_rx1_source)
		regmap_update_bits(cs35l41->regmap, CS35L41_DSP1_RX1_SRC,
				0xffffffff, cs35l41->pdata.dsp1_rx1_source);

	if (cs35l41->pdata.dsp1_rx2_source)
		regmap_update_bits(cs35l41->regmap, CS35L41_DSP1_RX2_SRC,
				0xffffffff, cs35l41->pdata.dsp1_rx2_source);
				...
				struct cs35l41_platform_data *pdata)
{
	...
	if (ret < 0) {
		dev_err(dev, "Device reg not found\n");
		return -EINVAL;
	} else if (val < 0 || val > 2) {
		dev_err(dev,
				"Invalid reg %d\n", val);
	...
	ret = of_property_read_u32(np, "cirrus,dsp1-rx1-source", &val);
	if (ret >= 0)
		pdata->dsp1_rx1_source = val;

	ret = of_property_read_u32(np, "cirrus,dsp1-rx2-source", &val);
	if (ret >= 0)
		pdata->dsp1_rx2_source = val;
				...
				struct cs35l41_platform_data *pdata)
{
	...
	dev_info(cs35l41->dev, "cs35l41 probe\n");
	...
	/* invalid reset GPIO can be an option if in stereo mode */
	cs35l41->reset_gpio =
		of_get_named_gpio(cs35l41->dev->of_node, "reset-gpios", 0);
	if (!gpio_is_valid(cs35l41->reset_gpio)) {
		dev_err(cs35l41->dev,
			"Failed to get reset GPIO: %d\n", cs35l41->reset_gpio);
	} else {
		ret = devm_gpio_request_one(
			cs35l41->dev, cs35l41->reset_gpio,
			GPIOF_DIR_OUT, "cs35l41_reset");
		if (ret < 0) {
			dev_err(cs35l41->dev, "Failed to request reset GPIO: %d\n", ret);
			cs35l41->reset_gpio = -1;
		} else {
			usleep_range(2000, 2100);
			gpio_set_value(cs35l41->reset_gpio, 1);
			usleep_range(2000, 2100);
		}
	}
